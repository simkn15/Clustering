...
hc <- hclustDivisiveGap(simMatrix, proteins, step, minSplits, maxSplit, minThreshold, maxThreshold, binarySearch = FALSE, GAP = TRUE, dimensions = 5)
# plot(hc$hc, xlab = "protein", ylab = "height")
plot(hc$hc, hang = -1, xlab = "Protein", ylab = "Height")
g <- plotCostDifference(hc$gap)
ggdraw(g)
pdf("plotCostGapSmallDataStep5K5Seed42NoBinary.pdf")
print(ggdraw(g))
dev.off()
